...
const SPRINT_MULT = 1.8; // <-- Sprint con Shift
...
const BULLET_BASE_AXIS = new THREE.Vector3(0, 1, 0); // eje local "hacia delante" del proyectil (Y)

function createBulletMesh() {
  // Mantiene la misma física (collider esférico). Solo cambia el aspecto visual a "bala".
  const r = SPHERE_RADIUS;
  const bodyRadius = r * 0.55;    // grosor de la bala
  const bodyLen    = r * 2.4;     // cuerpo
  const tipLen     = r * 1.2;     // punta
  const segs       = 14;
  ...
  // Punta cónica (apunta a +Y en local)
  ...
  // Tapa trasera (casquillo redondeado)
...
const spheres = []; let sphereIdx = 0;
for (let i=0; i<NUM_SPHERES; i++) {
  const mesh = createBulletMesh(); // <<< aspecto tipo bala
  scene.add(mesh);
  spheres.push({
    mesh,
...
// ================== HUD & Combat ==================
const PLAYER_MAX_HP = 500, PLAYER_ATTACK_DAMAGE = 25, PLAYER_ATTACK_RANGE = 1.4, PLAYER_ATTACK_COOLDOWN = 0.5;
let playerHP = PLAYER_MAX_HP, lastPlayerAttackTime = -Infinity;

const hud = document.createElement('div');
Object.assign(hud.style, { position:'absolute', left:'10px', bottom:'10px', color:'#fff', font:'14px/1.2 system-ui, sans-serif', textShadow:'0 1px 2px #000' });
hud.innerHTML = 'HP: 500 | Enemigos: 0';
...
function updateCrosshairVisibility(){ crosshair.style.opacity = gameStarted ? '1' : '0'; }

// ================== Utils ==================
const clamp=(x,min,max)=>Math.max(min,Math.min(max,x));
function closestPointOnSegment(a,b,p) {
  const ab=b.clone().sub(a);
  const t=THREE.MathUtils.clamp(ab.dot(p.clone().sub(a)) / Math.max(ab.lengthSq(),1e-6), 0, 1);
...
container.addEventListener('mousedown', ()=>{
  if (!gameStarted) return; // no pointer lock hasta iniciar
  document.body.requestPointerLock(); mouseTime = performance.now();
...
document.addEventListener('keydown', (ev)=>{ if(ev.code==='KeyF') playerAttack(); }, false);
...
window.startGame = function startGame(){
  ...
  if (assetsReady) { waveIndex=0; startWave(0); }
...
function throwBall(){
  ...
  s.collider.center.copy(playerCollider.end).addScaledVector(playerDirection, playerCollider.radius*1.5);
  const impulse = 15 + 30 * (1 - Math.exp((mouseTime - performance.now()) * 0.001));
  s.velocity.copy(playerDirection).multiplyScalar(impulse);
  s.velocity.addScaledVector(playerVelocity, 2);

  // Orientar visual de la bala en la dirección de tiro (solo visual, no cambia física)
...
function updatePlayer(dt){
  ...
  playerCollisions();
  camTarget.copy(playerCollider.end); camTarget.y += CAM_EYE_OFFSET;
}
...
const projectiles=[], FIREBALL_RADIUS=0.09, FIREBALL_SPEED=12, FIREBALL_TTL=4.0;
const fireballGeom = new THREE.SphereGeometry(FIREBALL_RADIUS, 16, 16);
const fireballMat  = new THREE.MeshStandardMaterial({ color:0xff7a00, emissive:0x552200, emissiveIntensity:1.2 });
...
// ================== LASERS (enemigos) ==================
const lasers = [];
const LASER_RADIUS   = 0.035;   // grosor del haz
const LASER_TTL      = 0.12;    // duración visual de cada pulso (s)
const LASER_COOLDOWN = 1.0;     // cadencia (s)
const LASER_RANGE    = 18;      // alcance
const LASER_DPS      = 24;      // daño por segundo (se integra por pulso)
...
const laserGeom = new THREE.CylinderGeometry(LASER_RADIUS, LASER_RADIUS, 1, 12); // eje Y
...
const SPAWN_NEAR_MIN = 6;   // mín ~6u
const SPAWN_NEAR_MAX = 20;  // máx ~20u
...
const LEVEL_MARGIN_XZ = 25; // evita bordes
let innerBox = null;         // caja interior segura
let constructionBoxes = [];  // cajas de construcción
...
const CLIP_FILES = { Idle:'Idle.fbx', Walk:'Walk.fbx', Run:'Run.fbx', Attack:'Attack.fbx', Hit:'Hit.fbx', Die:'Die.fbx' };
async function loadEnemyAssets(){
  ...
  for (const [name,file] of Object.entries(CLIP_FILES)) {
    try { const f = await fbxEnemyLoader.loadAsync(file); if (f.animations && f.animations[0]) ENEMY_CLIPS[name] = f.animations[0]; }
    catch { console.warn(`[Anim] falta clip ${file}`); }
  }
  ENEMY_READY = true; return true;
}

// ===== Visión / awareness
function hasLineOfSight(from,to){
  const dir = new THREE.Vector3().subVectors(to,from);
  const len = dir.length(); if (len < 1e-6) return true;
...
function canSeePlayer(enemyCenter, enemyForward){
  ...
  const eye = enemyCenter.clone().add(new THREE.Vector3(0,0,0)); // solo para FOV; LOS se comprueba abajo
...
class Enemy{
  constructor(x,z, tag){
    ...
    this.attackRange=1.2; this.attackDamage=10; this.attackCooldown=1.0;
    this.lastAttackTime=-Infinity; this.lastShootTime=-Infinity;
    this.lastLaserTime=-Infinity;
    this.hp=60; this.dead=false; this.tag=tag||null;
    this.dieEndTime=null;

    this.state='Idle'; this.forward=new THREE.Vector3(0,0,1);
    this.model=null; this.mixer=null; this.actions={};
    this._yVis=0; this._yVel=0;
    this.headOffsetY = 1.2; // offset por defecto (se recalcula con bbox)

    loadEnemyAssets().then(() => {
      if (ENEMY_BASE) this.model = SkeletonUtils.clone(ENEMY_BASE);
      else {
        const m = new THREE.Mesh(new THREE.CapsuleGeometry(0.25,0.8,4,8), new THREE.MeshStandardMaterial({ color:0xff3b3b }));
        m.castShadow = m.receiveShadow = true; this.model = m;
      }
      ...
      if (Object.keys(ENEMY_CLIPS).length && this.model && ENEMY_BASE) {
        ...
        ['Attack','Hit','Die'].forEach(n=>{ const a=this.actions[n]; if(a){ a.setLoop(THREE.LoopOnce,1); a.clampWhenFinished=true; }});
      }
      ...
      // ==== Calcular offset de cabeza a partir del bounding box ====
      try {
        const box = new THREE.Box3().setFromObject(this.model); // en coords de mundo
        const alturaDesdeBase = box.max.y - this.model.position.y;
        this.headOffsetY = Math.max(0.9, alturaDesdeBase * 0.92 - 0.02);
      } catch(e) { /* fallback 1.2 */ }
    });
  }

  center(){ return new THREE.Vector3().addVectors(this.collider.start,this.collider.end).multiplyScalar(0.5); }
  ...
  setAnimWeights(idleW, walkW){
    ...
    if (this.actions.Walk) {
      ...
      const tScale = THREE.MathUtils.clamp(vH/(1.4||1.0), 0.6, 1.6);
  ...
  shootEyeLasers(now){
    ...
    // Base "cráneo": posición del modelo + offset precomputado de cabeza
    ...
    if (los){
      ...
      const dmgPulse = (LASER_DPS * LASER_TTL) * 0.5; // dos ojos → mitad cada uno
  ...
  tryShoot(toPlayer, now){
    ...
    const dist = toPlayer.length(); if (dist<SHOOT_MIN || dist>SHOOT_MAX) return false;
    const mid=this.center(); const eye = mid.clone().add(new THREE.Vector3(0,0.9,0));
    ...
    this.lastShootTime = now; return true;
  }
  ...
  update(delta){
    ...
    if (this.state==='Idle') {
      ...
      if (sees) {
        // 1) Intento con láser de ojos (prioridad)
        const usedLaser = this.shootEyeLasers(clock.elapsedTime);

        // 2) Si no hubo láser, intentamos proyectil
...
function rayGround(x,z){ return worldOctree.rayIntersect(new THREE.Ray(new THREE.Vector3(x,1000,z), new THREE.Vector3(0,-1,0))); }
function groundYAt(x,z,fallbackY=5){ const hit=rayGround(x,z); return hit?hit.position.y:fallbackY; }
function sampleSpawnHeightThreshold(samples=600){
  ...
  for(let i=0;i<samples;i++){
    ...
    const hit=rayGround(x,z); if(!hit) continue;
...
function canPlaceHereParam(x,z, minNormalY, minYOrNull, minPlayerDist2 = SPAWN_NEAR_MIN*SPAWN_NEAR_MIN, maxPlayerDist2 = SPAWN_NEAR_MAX*SPAWN_NEAR_MAX){
...
function pickSpawnPointsGridPreferHigh(N, gridN=5){
  const pts=[]; if(!lastSceneBox) return pts;
  ...
  for (let gz=0; gz<gridN; gz++) {
    for (let gx=0; gx<gridN; gx++) {
      ...
      for (let t=0; t<18; t++){
        ...
        if (!canPlaceHereParam(x, z, MIN_GROUND_NORMAL_Y_STRICT, SPAWN_MIN_Y, SPAWN_NEAR_MIN*SPAWN_NEAR_MIN, SPAWN_NEAR_MAX*SPAWN_NEAR_MAX)) continue;
        const y = groundYAt(x,z,0);
        let ok = true;
        for (const p of pts) if (p.distanceToSquared(new THREE.Vector3(x,y,z)) < MIN_SEPARATION*MIN_SEPARATION) { ok=false; break; }
        if (ok) { chosen = new THREE.Vector3(x,y,z); break; }
      }
...
// ---- Fallbacks con sesgo a construcción ----
  ...
  while (pts.length < N && tries < 3000){
    ...
    if (hit){
      ...
      if (d2 >= SPAWN_NEAR_MIN*SPAWN_NEAR_MIN && d2 <= SPAWN_NEAR_MAX*SPAWN_NEAR_MAX){
        let ok=true;
        for (const p of pts) if (p.distanceToSquared(new THREE.Vector3(x,y,z)) < (4*4)) { ok=false; break; }
...
function forceSpawnRingAroundPlayer(N){
  const pts=[]; let tries=0; const base=playerCollider.end.clone();
  while (pts.length<N && tries<1800){
    const ang=Math.random()*Math.PI*2; const r=THREE.MathUtils.randFloat(SPAWN_NEAR_MIN, Math.max(SPAWN_NEAR_MIN+1, SPAWN_NEAR_MAX*0.9));
    const x=base.x+Math.cos(ang)*r; const z=base.z+Math.sin(ang)*r; const hit=rayGround(x,z);
    if (hit){
      const tag = groundTagAt(x,z);
      if (tag === TAGS.GRASS) { tries++; continue; }
      if (!isNearConstructionXZ(x,z, 10)) { tries++; continue; }
      if (!isInsideInnerBoxXZ(x,z)) { tries++; continue; }
      const y=hit.position.y; let ok=true;
      for (const p of pts) if (p.distanceToSquared(new THREE.Vector3(x,y,z)) < (4.5*4.5)) { ok=false; break; }
      if (ok) pts.push(new THREE.Vector3(x,y,z));
    }
    tries++;
  }
...
function spawnEnemiesFromPoints(pts, tag){ for (const p of pts) spawnEnemyOnFloor(p.x,p.z,tag); }
function spawnEnemiesDistributedSafe(N, tag){
  ...
  let guard=0; while (pts.length < N && guard < 10){ pts = pts.concat(forceSpawnRingAroundPlayer(N - pts.length)); guard++; }
  if (pts.length === 0) {
...
// ====== Gestor de OLEADAS (3 máximo) ======
function aliveCount(){ return enemies.filter(e=>!e.dead && e.model!==null).length; }
function aliveInWave(tag){ return enemies.some(e=>e.tag===tag && !e.dead && e.model!==null); }
...
loader.load('clock_tower_free_fire_model.glb', async (gltf)=>{
  ...
  // === Etiquetado + cajas de construcción ===
  ...
  assetsReady = true;
  if (gameStarted || pendingStart) { waveIndex = 0; startWave(0); }
  updateCrosshairVisibility();
});
...
function animate(){
  ...
  // Render pasivo mientras no se inicie el juego
  if (!gameStarted){
  ...
  for (let i=0; i<STEPS_PER_FRAME; i++){
    ...
    updateLasers(dt);          // <— láseres
    teleportPlayerIfOob();

    for (const e of enemies){ if (e.model) { e.update(dt); resolvePlayerEnemyPush(e); } }
  }
  ...
  if (waveActive) {
    ...
    if (!aliveInWave(currentTag)) {
      ...
      if (waveIndex+1 < WAVE_SIZES.length) {
        ...
        showBanner('¡Todas las oleadas completadas!', 3);
  ...
  hud.innerHTML = `HP: ${Math.ceil(playerHP)} | Enemigos: ${aliveCount()}`;

  // ==== GAME OVER (mensaje y respawn) ====
  if (playerHP <= 0) {
...
function resolvePlayerEnemyPush(e){
  ...
  if (d < r) {
    ...
    const vnP = n.dot(playerVelocity); if (vnP < 0) playerVelocity.addScaledVector(n, -vnP*0.7);
    const vnE = n.dot(e.velocity);     if (vnE > 0) e.velocity.addScaledVector(n, -vnE*0.7);
  }
}
...
function updateSpheres(dt){
  ...
  for (const s of spheres) {
    ...
    // Orientación visual de la bala según su velocidad (no afecta a la física)
